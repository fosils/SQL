...
select task_bots.give_task_bot(11959)
select * from task_bots.logs
--delete from task_bots.logs
select parameters,parameters->>'worker_initials' from task_manager.tasks where id = 11959;
...
declare
	...
	begin
		...
		insert into task_bots.logs(task_id,botname,"action","result") values (task_id,bot_name,'CREATE TASK FOR USER '||worker_initials||' task id ='||cast(created_task_id  as text),'success');
		update task_manager.tasks set task_status='completed' where id = task_id;
		return true;
	exception when others then
		insert into task_bots.logs(task_id,botname,"action","result") values (task_id,bot_name,SQLERRM,'failed');
